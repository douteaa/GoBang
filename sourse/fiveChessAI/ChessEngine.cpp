...
char AIboard[BOARD_WIDTH][BOARD_WIDTH];
int winner;     //胜出者
...
//根据位置评分，其中AIboard是当前棋盘，p是位置，role是评分角色，比如role是Human则是相对人类评分，比如role是computer则是对于电脑评分
int evaluatePoint(char AIboard[BOARD_WIDTH][BOARD_WIDTH], Position p) {
    int result;
    int i, j;
    int role;
    ...
    for (i = max(0, p.x - 5); i < (unsigned int)min(BOARD_WIDTH, p.x + 6); i++) {
        if (i != p.x) {
            lines[0].push_back(AIboard[i][p.y] == role ? '1' : AIboard[i][p.y] == 0 ? '0' : '2');
            lines1[0].push_back(AIboard[i][p.y] == role ? '2' : AIboard[i][p.y] == 0 ? '0' : '1');
        }
        else {
            lines[0].push_back('1');
    ...
    for (i = max(0, p.y - 5); i < (unsigned int)min(BOARD_WIDTH, p.y + 6); i++) {
        if (i != p.y) {
            lines[1].push_back(AIboard[p.x][i] == role ? '1' : AIboard[p.x][i] == 0 ? '0' : '2');
            lines1[1].push_back(AIboard[p.x][i] == role ? '2' : AIboard[p.x][i] == 0 ? '0' : '1');
        }
        else {
            lines[1].push_back('1');
    ...
    for (i = p.x - min(min(p.x, p.y), 5), j = p.y - min(min(p.x, p.y), 5); i < (unsigned int)min(BOARD_WIDTH, p.x + 6) && j < (unsigned int)min(BOARD_WIDTH, p.y + 6); i++, j++) {
        if (i != p.x) {
            lines[2].push_back(AIboard[i][j] == role ? '1' : AIboard[i][j] == 0 ? '0' : '2');
            lines1[2].push_back(AIboard[i][j] == role ? '2' : AIboard[i][j] == 0 ? '0' : '1');
        }
        else {
            lines[2].push_back('1');
    ...
    for (i = p.x + min(min(p.y, BOARD_WIDTH - 1 - p.x), 5), j = p.y - min(min(p.y, BOARD_WIDTH - 1 - p.x), 5); i >= (unsigned int)max(0, p.x - 5) && j < (unsigned int)min(BOARD_WIDTH, p.y + 6); i--, j++) {
        if (i != p.x) {
            lines[3].push_back(AIboard[i][j] == role ? '1' : AIboard[i][j] == 0 ? '0' : '2');
            lines1[3].push_back(AIboard[i][j] == role ? '2' : AIboard[i][j] == 0 ? '0' : '1');
        }
        else {
            lines[3].push_back('1');
...
//局面评估函数，给一个局面评分
int evaluate(char AIboard[BOARD_WIDTH][BOARD_WIDTH], Role role) {
...
void updateScore(char AIboard[BOARD_WIDTH][BOARD_WIDTH], Position p) {
...
//alpha-beta剪枝
int abSearch(char AIboard[BOARD_WIDTH][BOARD_WIDTH], int depth, int alpha, int beta, Role currentSearchRole) {
    ...
    int score1 = evaluate(AIboard, currentSearchRole);
    int score2 = evaluate(AIboard, currentSearchRole == HUMAN ? COMPUTOR : HUMAN);
    ...
    //set<Position> possiblePossitions = createPossiblePosition(AIboard);
    ...
    for (iter = tmpPossiblePositions.begin(); iter != tmpPossiblePositions.end(); iter++) {
        possiblePositions.insert(Position(iter->x, iter->y, evaluatePoint(AIboard, *iter)));
    }
    ...
    while (!possiblePositions.empty()) {
        ...
        //放置棋子
        AIboard[p.x][p.y] = currentSearchRole;
        currentZobristValue ^= boardZobristValue[currentSearchRole - 1][p.x][p.y];
        updateScore(AIboard, p);

        //增加可能出现的位置
        p.score = 0;
        ppm.AddPossiblePositions(AIboard, p);

        int val = -abSearch(AIboard, depth - 1, -beta, -alpha, currentSearchRole == HUMAN ? COMPUTOR : HUMAN);
        /*if (depth == DEPTH)
...
//获得下一步的走法
Position getAGoodMove(char AIboard[BOARD_WIDTH][BOARD_WIDTH]) {
    int score = abSearch(AIboard, DEPTH, MIN_SCORE, MAX_SCORE, COMPUTOR);
...
//输出棋盘
void printBoard(char AIboard[BOARD_WIDTH][BOARD_WIDTH]) {
    int i, j;
    for (i = 0; i < BOARD_WIDTH; i++) {
        for (j = 0; j < BOARD_WIDTH; j++) {
            cout << (int)AIboard[i][j] << " ";
        }
        cout << endl;
    }
}
...
//在开始之前，一些初始化工作
void beforeStart() {
    memset(AIboard, EMPTY, BOARD_WIDTH * BOARD_WIDTH * sizeof(char));
...
string takeBack() {
    if (moves.size() < 2) {
        ...
        for (i = 0; i < BOARD_WIDTH; i++) {
            for (j = 0; j < BOARD_WIDTH; j++) {
                resultStr.push_back(AIboard[i][j] + 48);
            }
        }
    ...
    AIboard[previousPosition.x][previousPosition.y] = EMPTY;
    updateScore(AIboard, previousPosition);
...
string reset(int role) {
    ...
    //电脑先走
        ...
        AIboard[7][7] = COMPUTOR;
        updateScore(AIboard, Position(7, 7));

        moves.push(Position(7, 7));
        searchResult = Position(7, 7);

        ppm.AddPossiblePositions(AIboard, Position(7, 7));
...
string nextStep(int x, int y) {
    ...
    updateScore(AIboard, Position(x, y));

    //增加可能出现的位置
    ppm.AddPossiblePositions(AIboard, Position(x, y));

    Position result = getAGoodMove(AIboard);

    AIboard[result.x][result.y] = COMPUTOR;
    currentZobristValue ^= boardZobristValue[COMPUTOR - 1][result.x][result.y];
    updateScore(AIboard, result);

    //增加可能出现的位置
    ppm.AddPossiblePositions(AIboard, result);
