...
void initName()
{
	...
	for (int i = 0; i < PROGRAMMER_NUM; i++)
	{
		...
		programmer[i] = { "田露涛", rand() % (dwSizeX - 3), rand() % dwSizeY, vx, vy, FOREGROUND_RED | FOREGROUND_INTENSITY };
	}
	programmer[0].name = "田露涛";
	programmer[1].name = "林益杰";
	programmer[2].name = "蔡林睿";
	programmer[1].color = FOREGROUND_GREEN | FOREGROUND_INTENSITY;
	programmer[2].color = FOREGROUND_BLUE | FOREGROUND_INTENSITY;
}
